...
app/
  ...
  api/
    ...
    xero/connect/route.ts        Xero OAuth — GET only, redirects to Xero's consent screen
    xero/callback/route.ts       Xero OAuth — GET only, exchanges the code, stores the connection
...
lib/
  ...
  utils.ts                       cn() etc.
  xero/
    oauth.ts                     PKCE + signed single-use state, authorisation URL, token exchange
    crypto.ts                    AES-256-GCM encryption for refresh tokens at rest, state HMAC
    client.ts                    Authenticated fetch wrapper; write gate (Section 6/7)
    mapping.ts                   Gilbert OS invoice → Xero payload mapping
    types.ts                     Xero API / connection types
...
| Migrations | `scripts/migrate-base.mjs`, `scripts/migrate-funding.mjs`, `scripts/migrate-xero.mjs` |
| API routes / server actions | `app/api/*`, `app/actions/*` |
| Major UI components | `components/*` |
| Xero integration | `lib/xero/*`, `app/api/xero/*`, `scripts/xero-*` |
...
`scripts/migrate-base.mjs` (core) + `scripts/migrate-funding.mjs` (funding) +
`scripts/migrate-xero.mjs` (Xero connection layer). 19 tables.
...
| 3 | `scripts/migrate-xero.mjs` | 4 Xero connection-layer tables (`xero_connections`, `xero_oauth_state`, `xero_sync_state`, `xero_account_map`) | **Applied 2026-08-12** | Yes | `node --env-file=.env.development.local scripts/migrate-xero.mjs` |
...
>
> **`migrate-xero.mjs` has been applied to production** (12 statements, 2026-08-12).
> Verified directly against the live database: all four `xero_*` tables exist, their
> columns and indexes match both the DDL and `lib/db/schema.ts`, and
> `xero_connections` holds the live Gilbert Build Co tenant. Every statement is
> `IF NOT EXISTS`, so a re-run is a safe no-op — but any *further* schema change
> must be a NEW statement appended to the `DDL` array, never an edit to one already
> applied.
...
| `scripts/xero-dry-run.mts` | **Read-only.** Maps every confirmed invoice through `mapInvoiceToXeroBill` and prints the would-be payload + gap report; no network calls, no writes anywhere | Yes (read-only) | `npx tsx`; Phase 1 mapping validation only |
| `scripts/xero-verify-connection.mts` | **Read-only.** Confirms an already-connected Xero org is reachable (GET org + chart of accounts) | Yes (read-only) | `npx tsx`; requires a completed OAuth connection already in `xero_connections` |

**Fresh database rebuild = the schema migrations in order above.** The data scripts are
only needed to reconstruct Higher Farm's funding budget (and, once connected, the Xero
account map) in a brand-new database; they are already applied in production (except
`migrate-xero.mjs` — see the note above) and are safe to re-run.
...
| `XERO_CLIENT_ID` | **Yes** (for the Xero flow) | Xero app client ID. Public identifier, safe to reference in code/comments. Read by `lib/xero/oauth.ts`. |
| `XERO_CLIENT_SECRET` | **Yes** (for the Xero flow) | Xero app client secret. The owner pastes it in directly; never generated, written to a file, or logged by this codebase. Every OAuth call fails loudly if it is absent. |
| `XERO_REDIRECT_URI` | **Yes** (for the Xero flow) | Must exactly match the redirect URI registered on the Xero app. Currently set to a `localhost` value locally — needs the production URL added on the Xero app, and this var set to it in Vercel, before the OAuth flow works in production. |
| `XERO_TOKEN_ENCRYPTION_KEY` | **Yes** (for the Xero flow) | 32-byte secret (base64 or hex) deriving the AES-256-GCM key that encrypts refresh tokens at rest and the HMAC key that signs the OAuth `state` parameter. Never generated or stored by this codebase — set it yourself (`openssl rand -base64 32`). Every function needing it throws immediately if unset or the wrong length. |
| `XERO_WRITE_ENABLED` | No — defaults to off | Must be the exact string `"true"` to let any non-GET call through `lib/xero/client.ts`. Off by default because Gilbert OS has no auth layer and is deployed on a public URL; flipping it is a deliberate environment change, never a code path. |
| `XERO_ALLOWED_TENANT_ID` | No — optional | Pins the deployment to one Xero organisation. `/api/xero/callback` is unauthenticated, so without a pin (and with no connection stored yet) the first caller to finish the flow decides which org connects — trust-on-first-use. Read by `lib/xero/oauth.ts`. Not documented in the original Xero task brief; found while reviewing the code for this pass. |
...
| --- | --- | --- | --- | --- |
| **Neon Postgres** | All application data | `DATABASE_URL` connection string | Link the same Neon project (or set `DATABASE_URL` to it). To rebuild elsewhere: run the schema migrations (Section 5). | **Yes — the entire production database.** |
...
| **Xero** | Accounting sync (connection layer only as of this pass — see `lib/xero/*`) | OAuth 2.0 (`XERO_CLIENT_ID`/`XERO_CLIENT_SECRET`), refresh tokens encrypted at rest with `XERO_TOKEN_ENCRYPTION_KEY` | Register/confirm the redirect URI on the Xero app for each environment; reconnect via `/api/xero/connect`. Writes to Xero are gated off unless `XERO_WRITE_ENABLED === "true"`. | `xero_connections`/`xero_oauth_state`/`xero_sync_state`/`xero_account_map` tables only — no accounting data has been synced as of this pass. |
